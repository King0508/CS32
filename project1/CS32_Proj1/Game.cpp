    ...
    while (!p->isPassedOut() && m_city->tooterCount() > 0)
    {
        ...
        else
        {
            ...
            case 'r':
                ...
                break;
            case 'h':
                (m_city->history()).display();//in the case of 'h',the pointer to the city call the history function to return a referenced history object, which then calls its display function that displays the history grid 
        ...
        if (action[0] != 'h')//we do not intend for 'h' to take up a turn, so moveTooters() is only called when the user input is valid and non 'h' 
        {
            m_city->moveTooters();
