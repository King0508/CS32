...
void City::preachToTootersAroundPlayer()
{
    ...
    for (int k = 0; k < m_nTooters; )
    {
        ...
        {
            m_history.record(tp->row(), tp->col()); //each time a tooter is preached, its location is recorded using the .record() function in the history class 
            delete m_tooters[k];
...
{
    return m_history; //returns a reference history object 
}
