...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
    ...
    for (vector<Student>::iterator stu = s.begin() + 1; stu != s.end(); stu++) {
        Student nextStu = *stu;
        vector<Student>::iterator prevStu = stu;
        while (prevStu != s.begin() && comp(nextStu, *(prevStu - 1))) {
            *prevStu = *(prevStu - 1);
            --prevStu;
        }
        *prevStu = nextStu;
...
void sortUsingPtrs(vector<Student>& students, bool comp(const Student*, const Student*))
{
    ...
    //        order.
    vector<Student*>::iterator p = studentPtr.begin();
    vector<Student>::iterator s = students.begin();
    for (; p!= studentPtr.end() || s != students.end(); p++, s++)
        *s = **p;
